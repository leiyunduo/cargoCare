...
;(function(){
  ...
  $('.d_first_transfer_code').on('change', function () {
    LXHR.POST(url, {code: $(this).val(), type: 1,}).done(res => {
      if(res.status === 200) {
        res.data && $('.d_first_transfer_name').val(res.data[0])
  ...
  $('.d_second_transfer_code').on('change', function () {
    LXHR.POST(url, {code: $(this).val(), type: 1,}).done(res => {
      if(res.status === 200) {
        res.data && $('.d_second_transfer_name').val(res.data[0])
  ...
  $('.d_product_no').on('change', function () {
    LXHR.POST(url, {code: $(this).val(), type: 2,}).done(res => {
      if(res.status === 200) {
  ...
  $('.d_product_category_code').on('change', function () {
    LXHR.POST(url, {code: $(this).val(), type: 3,}).done(res => {
      if(res.status === 200) {
  ...
  $('.d_packaging_code').on('change', function () {
    LXHR.POST(url, {code: $(this).val(), type: 4,}).done(res => {
      if(res.status === 200) {
...
let fillDetail = d => {
   ...
   // $('.d_transshipment_unit').val(d.transshipment_unit)                                      // 中转理货单位
    ...
    $('.d_receipt_type').val(d.d_receipt_type)
