...
// 开单费用==================================================================================================
;(function(){
  $('.detail_destination_code').change(function (evt) {
    LXHR.POST(getCaPriceSpeciesUrl, {destination_code:$(this).val()}).done(res => {
      if(res.status === 200) {
  ...
  $('.ca_cost_tab').on('click', function () {
    !costInfo.costList.length && getCostInfo()
  })
})()
...
;(function(){
  let getVolumeInfo = () => {
    ...
    if(!volumeInfo.volumeList.length){
      pageInfo.isEdit ? LXHR.POST(queryVolumeUrl, {"order_no": pageInfo.edit_order_no}).done(res => {
        if(res.status === 200){
...
;(function(){
  ...
  let getServiceInfo = () => {
    ...
    if(!customInfo.customList.length){
      pageInfo.isEdit ? LXHR.POST(queryCustServiceRecordUrl, {"order_no": pageInfo.edit_order_no, cust_service_type: 3}).done(res => {
        if(res.status === 200){
  ...
  // 切换到出发、到达信息选项卡=================================================================================
  $('.start_arrival').on('click',function(){
    pageInfo.edit_order_no && getStartInfo()
    arriveInfo.change(pageInfo.edit_order_no)
    getServiceInfo()
  })
...
;(function(){
  ...
  $('.detail_expected_flight_time').on('keyup', function (evt) {
    if(evt.keyCode === 13){
      gotoRelateFlight()
      getBookingInfo()
    }
