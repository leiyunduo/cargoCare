...
$('#edit_close_two_level_page').on('click', function () {
    $('.edit_two_level_menu_wrap input').val('')
    $('.two_level_menu_wrap').hide()
    $(".item").removeClass('active')
    $(".tabBox").hide()
    $(".msg").show()
...
// 渲染表格
        ...
        let str = data.map((item, index) => {
          ...
          <tr>
            ...
            <td class="flexCenter padding0 ">
              <div class="roundBg borderRaidus redBg"><i class="icon-loop margin-Left5" title="换单"></i></div>
              <div class="roundBg borderRaidus greenBg editBtn"><i class="edit icon margin-Left5" data-id="${item.order_no}" title="编辑"></i></div>
              <div class="roundBg borderRaidus tealBg"><i class="icon-printer margin-Left5" title="打印"></i></div>
            </td>
            <td>${item.order_no}</td>
    ...
    let searchMainList = () => {
        // 请求地址
        const reqUrl = baseUrl + '/def/receive/pageQueryReceiveMain/'
        // 请求参数
        let params = {
...
// 发送请求
    ...
    */
    $('.chaxun').click(function () {
        ...
        //运单号三字码校验
        function _reg(obj) {
            var _three = $(".order_three_code").val()
            var _threeCode = /^[A-Za-z0-9]{3}$/;
            var _shifa = $('.departure_code').val()
            var _shifaCode = /^[A-Z]{3}$/;
            var _mudi = $('.destination_code').val()
            var _mudiCode = /^[A-Z]{3}$/;
            if (_three == '') {
                console.log('加载1')
                if (_shifa == '') {
                    ...
                    if (_mudi == '') {
                        console.log('加载3')
                        mmmmmmm()
                    } else if (!_mudi == '' && _mudiCode.test(_mudi)) {// 只有第三个值
                        mmmmmmm()
                        console.log('格式正确加载3')
                    } else {
                        alert('目的站输入有误')
                    }
                } else if (!_shifa == '' && _shifaCode.test(_shifa)) { //第一个没值  第二个有值  格式正确
                    console.log('格式正确加载2')
                    if (_mudi == '') { //第三个没值 到这结束
                        mmmmmmm()
                        console.log('3的内容为空')
                    } else if (!_mudi == '' && _mudiCode.test(_mudi)) {   //第一个没值  第二个有值 第三个有值 格式正确
                        console.log('格式正确加载3' + _three + _shifa + _mudi)
                        mmmmmmm()
                    } else {
                        alert('目的站输入有误')
                    }
            ...
            } else if (!_three == '' && _threeCode.test(_three)) {//第一个有值  并且格式正确
                if (_shifa == '') {
                    console.log('2的内容为空')
                    if (_mudi == '') {
                        mmmmmmm()
                        console.log('3的内容为空')
                    } else if (!_mudi == '' && _mudiCode.test(_mudi)) { //第一个有值  第二个没有   第三个有
                        console.log('格式正确加载3')
                        mmmmmmm()
                    } else {
                        alert('目的站输入有误')
                    }
                } else if (!_shifa == '' && _shifaCode.test(_shifa)) {// 第一个有值  第二个有值  并且格式正确
                    console.log('格式正确加载2:' + _three + _shifa);
                    if (_mudi == '') {
                        console.log('3的内容为空')
                        mmmmmmm()
                    } else if (!_mudi == '' && _mudiCode.test(_mudi)) {
                        mmmmmmm()
                        console.log('格式正确加载3' + _three + _shifa + _mudi)//三个参数格式都正确
                    } else {
                        alert('目的站输入有误')
                    }
                } else {//第二个有值  格式错误
...
//!************通过验证****************!//
            function mmmmmmm() {
                ...
                //!********一级查询*********
                $.ajax({
                    url: baseUrl + '/def/receive/pageQueryReceiveMain/',
                    data: _data,
                    type: "POST",
                    dataType: "json",
                    error: function () {
                        ...
                        if (e.status == '200') {
                            ...
                            if (e.data[0].list.length != '0') {
                                ...
                                for (i = 0; i < e.data[0].list.length; i++) {
                                    ...
                                    $(".shouhuozhu").append('<tr order="' + e.data[0].list[i].order_no + '"><td>' + (i + 1) + '</td><td class="flexCenter padding0 "><div class="roundBg borderRaidus redBg"><i class="icon-loop margin-Left5" title="换单"></i></div><div class="roundBg borderRaidus greenBg editBtn"><i class="edit icon margin-Left5" title="编辑"></i></div><div class="roundBg borderRaidus tealBg"><i class="icon-printer margin-Left5" title="打印"></i></div></td><td>' + e.data[0].list[i].order_no + '</td><td>' + e.data[0].list[i].departure + '</td><td>' + e.data[0].list[i].destination + '</td><td>' + e.data[0].list[i].state + '</td><td>' + e.data[0].list[i].sender_name + '</td><td>' + e.data[0].list[i].receiver_name + '</td><td>' + e.data[0].list[i].billing_time + '</td><td>' + e.data[0].list[i].receiver_telephone + '</td><td>' + e.data[0].list[i].departure_status + '</td><td>' + e.data[0].list[i].correspond_order_no + '</td><td>' + e.data[0].list[i].sender_telephone + '</td></tr>');
                                }//for
...
$('.q_Enter').keydown(function (e) {
    ...
    if (e.keyCode == 13) {
        ...
        if (1 <= Q_value && Q_value <= localStorage.allpage) {
            ...
            // LALERT.success('填写1~' + localStorage.allpage + '的值')
        }
...
$('._putong').click(function () {
    ...
    var num = localStorage.expected_flight_no
    var flight_num = num.slice(2, 6);
...
$('._menu_item').on('click', '._itemOne', function () {
   ...
   // localStorage.setItem('_weight', weight)
    localStorage.setItem('_price', $(this).attr("price"))
    console.log($(this).attr("title"));
  //  console.log(weight);
    $('._feilv').val(localStorage._invoice_price)
    $('.pricecode').val(localStorage._price)
    //跳转至开单费用选项卡
    cost()
...
function cost(){
    ...
    var weight = document.querySelector('.weight').value;
    var quantity = document.querySelector('.quantity').value;
    var easy_flag = document.querySelector('.easy_flag').value;
    var air_transit = document.querySelector('.air_transit').value;
    var sender_name = document.querySelector('#sender_name').value;
    var sender_code = document.querySelector('.sender_code').value;
    var first_transfer_name = document.querySelector('.first_transfer_name').value;
    var two_flight_code = document.querySelector('.expected_two_flight_code').value;
    var first_transfer_code = document.querySelector('.first_transfer_code').value;
    var destination_code = document.querySelector('.destination_code').value;
    var product_category = document.querySelector('.product_category').value;
    $.ajax({
        ...
        data: {
            ...
            "destination_code": destination_code,
            "two_flight_code": two_flight_code,
            "first_transfer_code": first_transfer_code,
            "first_transfer_name": first_transfer_name,
            "product_category": product_category,
...
$(".tianjia").click(function () {
    ...
    var receipt_type = document.querySelector('#receipt_type').value;
    ...
    var price_species_name = $("._putong>.text").text()
...
//*********_datas
    ...
    $.ajax({
        url: baseUrl + '/def/receive/insertReceiveMain/',
...
$('.w_table').on('keydown', '.M_add', function (e) {
    ...
    if (ev.keyCode == 13) {
        ...
        $(this).removeClass('M_add')
        $("._money").append('<tr this_index="'+(feiyongLength-1)+'"><td>' + (Number(localStorage.feiyongL) + feiyongLength) + '</td><td><input type="text"></td><td><input type="text"></td><td><input type="text"></td><td><input type="text"></td><td><input type="text"></td><td><input type="text"></td><td><input type="text"></td><td><input type="text"></td><td><input type="text"></td><td><input type="text"></td><td><input type="text" class="M_add"></td><td><button class="ui Medium red button shanqian">删除</button></td></tr>')
...
$('.talkerbox').on('keydown', '.seriver', function (e) {
    ...
    if (ev.keyCode == 13) {
        ...
        for (var i = 0; i < serL.length; i++) {
            ...
            var con = serL.eq(i).find('.content').val();
...
$("._del").click(function () {
    ...
    $.ajax({
        url: baseUrl + '/def/receive/deleteReceiveMain/',
...
function M_edit_ajax(M_Boss_number) {
    ...
    $(".msg").addClass('active')
    $.ajax({
        url: baseUrl + '/def/receive/beforeEditReceiveMain/',
        ...
        success: function (e) {
            ...
            if(e.status==200){
                ...
                $('.volume').val(e.data[0].volume);
                $('.weight').val(e.data[0].weight);
                $('.clearing_unit_').val(e.data[0].clearing_unit_code);
                $('.clearing_unit_code_').val(e.data[0].clearing_unit);
                $('.discount_').val(e.data[0].discount);
                $('.payment_method_').val(e.data[0].payment_method);
                $('#receipt_type').val(e.data[0].receipt_type);
                $('#state_').val(e.data[0].state);
                $('._billing_time').val(e.data[0].billing_time);
                $('._issuer').val(e.data[0].issuer);
                $('._billing_location').val(e.data[0].billing_location);
                ...
                $('.price_species_code_').val(e.data[0].price_species_code)
                $('#_putong')>$('.jiazhong').html(e.data[0].price_species_name)
                $('#_feilv').val(e.data[0].invoice_price)  ///费率
            }else{
                LALERT.msg(e.message)
            }
...
$('.editer').click(function () {
    //alert($('.service_record').val())
    // alert( $('.update_time').val())
    var _data = {
        ...
        "receipt_type": $('#receipt_type').val(),
        "state": $('#state_').val(),
        "billing_time": $('._billing_time').val(),
        "issuer": $('._issuer').val(),
        "billing_location": $('._billing_location').val(),
        ...
        "price_species_code":$('.price_species_code_').val(),
        "price_species_name":$('#_putong')>$('.jiazhong').html(),
}
    //保存时的验证
    let validate = $('.ui.form.edit_billing_details').form('is valid')
    console.log($('.ui.form.edit_billing_details').form('is valid'))
    if(!validate){
        $('.ui.form').form('validate form')
        return  //校验通过直接加载ajax
    }
    $.ajax({
        url: baseUrl + '/def/receive/editReceiveMain/',
 ...
 //打印
 $('.w_table').on('click', '.tealBg', function () {
     var num=$(this).parents().parents().attr('order')
     window.open( baseUrl + '/def/print/receiptMainSingle?order_no='+num+'')
 })
//收发货人
$('.sender_code_').blur(function () {
    $.ajax({
        url: baseUrl + '/def/basic/dic/getSenderReceiver',
        data: {"code": $('.sender_code_').val()},
        type: "POST",
        error: function () {
            //  LALERT.msg("服务器连接失败")
        },
        success: function (e) {
            if (e.status == 200) {
                $('#sender_name_').val(e.data[0].name)
                $('.sender_name_').val(e.data[0].name)
                $('.sender_telephone_').val(e.data[0].telephone)
                $('.sender_fax_').val(e.data[0].fax)
                $('.sender_address_').val(e.data[0].address)
                $('.sender_sms_notification_').val(e.data[0].sms_notification)
            }
        }
    });
})
$('.receiver_code_').blur(function () {
    $.ajax({
        url: baseUrl + '/def/basic/dic/getSenderReceiver',
        data: {"code": $('.receiver_code_').val()},
        type: "POST",
        error: function () {
            // LALERT.msg("服务器连接失败")
        },
        success: function (e) {
            if (e.status == 200) {
                //$('#fa').val(e.data[0].code)
                $('#receiver_name_').val(e.data[0].name)
                $('.receiver_name_').val(e.data[0].name)
                $('.receiver_telephone_').val(e.data[0].telephone)
                $('.receiver_fax_').val(e.data[0].fax)
                $('.receiver_address_').val(e.data[0].address)
                $('.receiver_sms_notification_').val(e.data[0].sms_notification)
                $('.delivery_mode_').val(e.data[0].delivery_mode)
            }
        }
    });
